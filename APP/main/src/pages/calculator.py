...
from PyQt5.QtWidgets import QPushButton, QLabel, QWidget, QLineEdit, QComboBox, QGraphicsDropShadowEffect, QMainWindow

from src.canvas.Canvas import Canvas
from src.lang.language import Language

class EquationWin(QMainWindow):

    def __init__(self, lang, parent=None):
        super(EquationWin, self).__init__()
    ...
    def setupUI(self):
        self.setGeometry((self.screen.width() - self.width) // 2, (self.screen.height() - self.height) // 2,
                                self.width, self.height)
        self.setFixedSize(self.width, self.height)

        self.setWindowTitle(self.lang["app-title"] + " \ " + self.lang["un"])

        self.start_widgets = QWidget(self)
        ...
        self.setCentralWidget(self.start_widgets)
